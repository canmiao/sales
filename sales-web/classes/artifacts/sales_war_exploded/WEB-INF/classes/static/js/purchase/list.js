$(document).ready(function () {
    ...
    $("#detail_tb").bootstrapTable({
        url: "http://localhost:8080/sales/purchase/select",
        method: 'get',
        pagination: true, //分页
        pageSize: 8, //每页显示8条
        singleSelect: false,
        ...
        columns: [
...

            {
                align: 'center', title: '操作', formatter: function (value, row) {
                    var text = "";
                    text = text + '<a href="javascript:void(0);" data-id="' + row.id + '" class="pass_purchase">审核通过</a>';
                    text = text + '<a href="javascript:void(0);" data-id="' + row.id + '" class="nopass_purchase">审核未通过</a>';
                    return text;
                }
            }
...
*/
    // 审核通过
    $("#detail_tb").on('click', '.pass_purchase', function (e) {
        e.preventDefault();
        $('#model-label').text('审核采购单信息');
        var id = $(this).data('id');
        $('#locationInfoModel').show();
        // 先清空下拉选择框
        $("#orderNumber").val('');
        ...
        $.ajax({
            url: 'http://localhost:8080/sales/purchase/manager/pass' + id,
            method: 'post',
            async: false,
            dataType: 'json',
            success: function (response) {
        ...
        url = "http://localhost:8080/sales/purchase/manager/pass"
    });

    // 审核通过
    $("#detail_tb").on('click', '.nopass_purchase', function (e) {
        e.preventDefault();
        ...
        $.ajax({
            url: 'http://localhost:8080/sales/purchase/manager/nopass' + id,
            method: 'post',
            async: false,
            dataType: 'json',
            success: function (response) {
        ...
        url = "http://localhost:8080/sales/purchase/manager/nopass"
    });
