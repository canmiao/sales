$(document).ready(function () {
    ...
    $("#detail_tb").bootstrapTable({
        ...
        columns: [
            ...
            {field: 'salerPhone', align: 'center', title: '销售员联系电话'},


            {
                align: 'center', title: '操作', formatter: function (value, row) {
                    var text = "";
                    text = text + '<a href="javascript:void(0);" data-id="' + row.id + '" class="modify_sales">审核</a>';
    ...
    $("#detail_tb").on('click', '.modify_sales', function (e) {
        ...
        $.ajax({
            url: 'http://localhost:8080/sales/salelist/update' + id,
            method: 'post',
            async: false,
            dataType: 'json',
            success: function (response) {
