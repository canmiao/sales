$(document).ready(function () {
    ...
    $("#detail_tb").bootstrapTable({
        url: "http://localhost:8080/sales/input/select",
        method: 'get',
        pagination: true, //分页
        pageSize: 8, //每页显示8条
        singleSelect: false,
        ...
        columns: [
            {checkbox: true},
            {field: 'orderNumber', align: 'center', title: '入库单号'},
            ...
            {field: 'incomingPart', align: 'center', title: '入库员'},
            {field: 'incomingPartPhone', align: 'center', title: '入库员联系电话'},
    ...
    $("#confirm_submit").click(function () {
        ...
        if (checkString($("#incomingPart").val())) {
            showToast("入库员不能为空...");
            return;
        }
        if (checkString($("#incomingPartPhone").val())) {
            showToast("入库员联系电话不能为空...");
            return;
        }
        ...
        var sales = {
            ...
            incomingPart: $("#incomingPart").val(),
            incomingPartPhone: $("#incomingPartPhone").val(),
            id: $("#id").val()
        };
        //调用保存方法
        $.ajax({
...
//动态绑定select标签中的option
        ...
        // 准备参数
        url = "http://localhost:8080/sales/input/insert"
    });

    // 打开修改页面
    $("#detail_tb").on('click', '.modify_sales', function (e) {
        e.preventDefault();
        $('#model-label').text('修改入库单信息');
        var id = $(this).data('id');
        $('#locationInfoModel').show();
        // 先清空下拉选择框
        $("#orderNumber").val('');
        ...
        $.ajax({
            url: 'http://locahost:8080/sales/input/select' + id,
            method: 'post',
            async: false,
            dataType: 'json',
            success: function (response) {
                ...
                $("#incomingPart").val(data.incomingPart);
                $("#incomingPartPhone").val(data.incomingPartPhone);
            }
        });
        url = "http://localhost:8080/sales/input/update"
    });
