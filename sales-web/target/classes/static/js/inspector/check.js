           ...
           // {field: 'checkNote', align: 'center', title: '备注'},

            {
                align: 'center', title: '操作', formatter: function (value, row) {
                    var text = "";
                    text = text + '<a href="javascript:void(0);" data-id="' + row.id + '" class="modify_check">审核</a>';
                    text = text + '<a href="javascript:void(0);" data-id="' + row.id + '" class="view_check">查看</a>';
                    return text;
                }
            }
...
*/
    // 审核通过
    $("#detail_tb").on('click', '.modify_check', function (e) {
        e.preventDefault();
        $('#model-label').text('审核表单');
        var id = $(this).data('id');
        $('#locationInfoModel').show();
        // 先清空下拉选择框
        $("#checkOrder").val('');
